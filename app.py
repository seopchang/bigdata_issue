...
Original file is located at
    https://colab.research.google.com/drive/1O9ruvpcr-Y_k71sbKqPo0SVxmllEnoob
"""
...
with st.sidebar:
    st.header("⚙️ 설정")
    # 사용자가 직접 API 키를 입력하도록 하여 보안 유지
    api_key = st.text_input("Google AI Studio API Key", type="password")

    st.markdown("---")
    # 자주 쓰시는 XAI를 기본 검색어로 설정해 두었습니다.
...
def get_papers(keyword, limit=5):
    """arXiv API에서 최신 논문 제목과 요약을 가져옵니다."""
    # 검색어를 URL 인코딩
...
if search_button:
    if not api_key:
        ...
        with st.spinner('데이터를 수집하고 AI가 분석 중입니다. 잠시만 기다려주세요...'):
            try:
                # API 키 설정
                ...
                papers_text = get_papers(search_query)
                ...
                2. 핵심 주제 3가지 (각 주제별로 간단한 설명 포함)
                3. 눈여겨볼 중요 키워드 5개 (해시태그 형태, 예: #머신러닝)
